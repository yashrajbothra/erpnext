...
class StockLedgerEntry(Document):
	...
	if TYPE_CHECKING:
		...
		custom_citi_no: DF.Data | None
	# end: auto-generated types

	def before_insert(self):
		if not self.custom_citi_no and self.voucher_detail_no:
			if self.voucher_type == "Sales Invoice":
				self.custom_citi_no = frappe.db.get_value("Sales Invoice Item", self.voucher_detail_no, "custom_citi_no")
			elif self.voucher_type == "Purchase Invoice":
				self.custom_citi_no = frappe.db.get_value("Purchase Invoice Item", self.voucher_detail_no, "custom_citi_no")
			elif self.voucher_type == "Stock Entry":
				self.custom_citi_no = frappe.db.get_value("Stock Entry Detail", self.voucher_detail_no, "custom_citi_no")
	...
	def validate_inventory_dimension_negative_stock(self):
		if self.is_cancelled or flt(self.actual_qty) >= 0:
			return
	...
	def validate_serial_batch_no_bundle(self):
		...
		if item_detail.has_serial_no or item_detail.has_batch_no:
			if flt(self.actual_qty) != 0 and not self.serial_and_batch_bundle:
	...
	def validate_batch(self):
		if self.batch_no and self.voucher_type != "Stock Entry":
			if (self.voucher_type in ["Purchase Receipt", "Purchase Invoice"] and flt(self.actual_qty) < 0) or (
				self.voucher_type in ["Delivery Note", "Sales Invoice"] and flt(self.actual_qty) > 0
			):
				return
