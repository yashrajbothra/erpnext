...
class JournalEntry(AccountsController):
	...
	def insert(self, *args, **kwargs):
		for row in self.accounts:
			...
			party_name = row.party or row.get("custom_party_custom") or self.get("custom_party_custom")
			if not party_name:
				continue

			if account_type == "Receivable":
				cust_id = self.get_or_create_customer(party_name)
				if cust_id:
					row.party_type = "Customer"
					row.party = cust_id
			elif account_type == "Payable":
				supp_id = self.get_or_create_supplier(party_name)
				if supp_id:
	...
	def get_or_create_customer(self, c_name):
		if not c_name:
			return None
		c_name = str(c_name).strip()
		existing = frappe.db.get_value("Customer", {"customer_name": c_name}, "name") or frappe.db.get_value("Customer", c_name, "name")
		if not existing:
			try:
	...
	def get_or_create_supplier(self, s_name):
		if not s_name:
			return None
		s_name = str(s_name).strip()
		existing = frappe.db.get_value("Supplier", {"supplier_name": s_name}, "name") or frappe.db.get_value("Supplier", s_name, "name")
		if not existing:
			try:
