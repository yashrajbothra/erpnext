...
def convert_excel(file_url):
    ...
    for row_idx, row in enumerate(sheet.iter_rows(min_row=1, values_only=True), start=1):
        ...
        is_date_row = any(re.search(fr'\b{x}\b', row_strs) for x in ["PORPOSEL", "PROPOSAL", "PORPOSAL", "PURPOSEL"])
        
        # If "TO" is present, only consider it a date row if it doesn't look like a data row
        # (Data rows usually have an account in Col 3 or Col 10)
        if not is_date_row and re.search(r'\bTO\b', row_strs):
            has_acc = (len(row) > 2 and row[2]) or (len(row) > 9 and row[9])
            if not has_acc:
                is_date_row = True
        ...
        if is_date_row or header_date is None:
            ...
            # Prioritize Column 12 (Index 11) per user request
            cells_to_check = []
            if len(row) > 11 and row[11]:
                cells_to_check.append(row[11])
            for cell in row:
        ...
        # Col 3:AC, 4:AMT, 5:PARTY, 6:DATE, 7:MODE, 8:REMARK
        ...
        l_mode   = row[6] if len(row) > 6 else None
    ...
    # Save output
    output_filename = f"kb_converted_ledger_{frappe.generate_hash(length=8)}.xlsx"
    output_path = frappe.get_site_path('public', 'files', output_filename)
    out_wb.save(output_path)
    
    # Register file in database
    with open(output_path, 'rb') as f:
        file_doc = save_file(output_filename, f.read(), 'Page', 'kb-excel-to-ledger', is_private=0)
